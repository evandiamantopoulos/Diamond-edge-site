...
launcher.addEventListener('click',()=>{win.style.display=(win.style.display==='flex'?'none':'flex')});document.getElementById('chat-close').addEventListener('click',()=>win.style.display='none');
const chatBody=document.getElementById('chat-body');const input=document.getElementById('chat-input');document.getElementById('chat-send').addEventListener('click',sendMsg);input.addEventListener('keydown',e=>{if(e.key==='Enter'){sendMsg();}});
...
async function sendMsg(){const q=input.value.trim();if(!q)return;addMsg(q,'user');input.value='';try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:q})});if(!r.ok)throw new Error('Network');const d=await r.json();addMsg(d.answer||'Sorry, no answer returned.','bot');}catch(err){addMsg(canned(q),'bot');}}
